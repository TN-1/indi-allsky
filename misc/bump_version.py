...
def set_version(new_version, suite="stable"):
    ...
    if BASE_HTML.exists():
        ...
        if count > 0:
            ...
            print(f" Updated {BASE_HTML.relative_to(ROOT_DIR)} (replaced {count} cache-buster{'s' if count != 1 else ''})")
        else:
            print(f" Warning: No cache-buster pattern matched in {BASE_HTML.relative_to(ROOT_DIR)}")
